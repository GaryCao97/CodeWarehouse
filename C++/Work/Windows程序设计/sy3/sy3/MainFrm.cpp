...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	lstBasicCommands.AddTail(ID_NAME_ZHANGSAN);
	lstBasicCommands.AddTail(ID_NAME_LISI);
	lstBasicCommands.AddTail(ID_SCORE_CHINESE);
	lstBasicCommands.AddTail(ID_SCORE_ENGLISH);
	lstBasicCommands.AddTail(ID_SCORE_MATH);
...
void CMainFrame::OnNameZhangsan()
{
	...
	m_pMainmenu = CMenu::FromHandle(m_wndMenuBar.GetDefaultMenu());
	CString strZS;
	if (m_bZhangSan) {
		strZS = _T("张三的动态菜单(&Z)");
		m_pMainmenu->InsertMenuW(4, MF_POPUP | MF_BYPOSITION | MF_STRING, (UINT)m_addMenu.GetSubMenu(0)->m_hMenu, strZS);
		m_wndMenuBar.CreateFromMenu(m_pMainmenu->GetSafeHmenu(), TRUE, TRUE);
		m_addMenu.Detach();
	}
	else {
