...
BOOL CAdminDlg::OnInitDialog()
{
	...
	{
		UserSet.GetODBCFieldInfo(i, DbFi);
		Lc.iSubItem = i - 1;
		Lc.fmt = LVCFMT_CENTER;
		if (DbFi.m_strName == _T("Username"))DbFi.m_strName = _T("用户名");
		else if (DbFi.m_strName == _T("Number"))DbFi.m_strName = _T("工号");
		else if (DbFi.m_strName == _T("Realname"))DbFi.m_strName = _T("员工姓名");
		else if (DbFi.m_strName == _T("Kind"))DbFi.m_strName = _T("工种");
		else if (DbFi.m_strName == _T("Duty"))DbFi.m_strName = _T("负责商品/种类");
...
void CAdminDlg::OnBnClickedSearchBtn()
{
	...
	if (m_sSearch == _T("")) {
		...
		if (!UserSet.IsOpen())
			UserSet.Open();
		UserSet.GetODBCFieldInfo(m_comboSearch.GetCurSel() + 1, info);
		if (UserSet.IsOpen())
			UserSet.Close();
		UserSet.m_strFilter = info.m_strName + _T("='") + m_sSearch + _T("'");
		CAdminDlg::PaintListItem();
	}
	UpdateData(FALSE);
}
...
void CAdminDlg::OnColumnclickUserlist(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (UserSet.IsOpen())
		UserSet.Close();
	UserSet.m_strSort = info.m_strName;
	CAdminDlg::PaintListItem();
	*pResult = 0;
}
