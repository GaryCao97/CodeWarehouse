...
public class PokerCardGame extends Application {
...
class CardGame {
	...
	public CardGame() {
		...
		for(int i = 0;i < 4;i++) {
			for(int j = 0;j < 13;j++) {
				gamePane.getChildren().add(cards[i*13 + (j)].CardImage);
				cards[i*13 + (j)].CardImage.setLayoutX(j*20);
				cards[i*13 + (j)].CardImage.setLayoutY(i*(cards[i*13 + (j)].CardImage.getFitHeight() + 10));
			}
		}
	}
...
class Poker extends CardGame {
	...
	public void deal(int n) {
		...
		if(n >= 2 && n <= 7) {
			...
			for(int i = 0;i < 5;i++) {
				for(int j = 0;j < n;j++) {
					gamePane.getChildren().add(cards[i*n + (j)].CardImage);
					cards[i*n + (j)].CardImage.setLayoutX(j*(cards[i*n + (j)].CardImage.getFitWidth()+10));
					cards[i*n + (j)].CardImage.setLayoutY(i*(cards[i*n + (j)].CardImage.getFitHeight() - 100));
				}
			}
		}
	}
